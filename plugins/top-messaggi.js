...
let handler = async (m, { conn, args, participants }) => {
    try {
        ...
        // Crea array con dati utente
        let usersData = [];
        
        for (let participant of participants) {
            if (participant.id === conn.user.jid) continue;
            
            const userData = users[participant.id] || {};
...
END:VCARD`
        ...
        conn.reply(m.chat, "❌ Errore nel comando top", m);
    }
};
