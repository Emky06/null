...
const LASTFM_API_KEY = '0f13829a53f7c7650562445e49063b34';
const BROWSERLESS_KEY = '2TdsfCQhO6hHvc062aa52ef9f252d478cde399ff9c1cb557c';
...
const handler = async (m, { conn, usedPrefix, command, text }) => {
    ...
    if (['cur', 'attuale', 'nowplaying', 'ciccione'].includes(command)) {
        try {
            ...
            const iosButtons = [
                ...
                { buttonId: `${usedPrefix}playaudio ${track.name} ${track.artist['#text']}`, buttonText: { displayText: "🎵 Audio" }, type: 1 },
                { buttonId: `${usedPrefix}playvideo ${track.name} ${track.artist['#text']}`, buttonText: { displayText: "📽️ Video" }, type: 1 }
            ];
    ...
    if (['topartists', 'topartisti'].includes(command)) {
        try {
            ...
            await sendListResponse(`🎤 *Top Artisti per @${user} (${periodInput})*`, cards);
