...
const STUB_TYPES = {
  ...
  172: '📝 Richiesta d\'ingresso al gruppo',
}
...
const isAdmin = m.isGroup ? await isUserAdmin(conn, m.chat, senderJid) : false

  const userId = senderJid.split('@')[0]

const isPrems = m.isGroup
    ? global.prems.includes(userId) || (global.db?.data?.groups?.[m.chat]?.prems || []).includes(userId)
    : global.prems.includes(userId)

  const roles = []
  if (isBot) roles.push(chalk.blueBright('Bot'))
...
╰━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╯`
  ...
  const out = await Promise.all(
    m.messageStubParameters.map(async param => {
      if (param.includes('@')) {
        const d = conn.decodeJid(param)
        const name = await getNameCached(d)
        return chalk.gray(formatJid(d, name || 'Sconosciuto'))
      } else {
